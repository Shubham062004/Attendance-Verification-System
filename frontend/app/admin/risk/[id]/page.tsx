...
function AdminRiskDetailContent({ params }: PageProps) {
    ...
    try {
      ...
      if (rec.location_validation_id) {
        const res = await fetch(
          `http://localhost:8000/location/validation/${rec.location_validation_id}`,
          { headers },
        );
      ...
      if (rec.verification_session_id) {
        const res = await fetch(
          `http://localhost:8000/verification/session/${rec.verification_session_id}`,
          { headers },
        );
  ...
  const getRiskBadge = (level: string) => {
    switch (level) {
      ...
      case "REVIEW":
        return (
          <span className="rounded-full border border-amber-500/20 bg-amber-500/10 px-3 py-0.5 text-xs font-bold text-amber-400">
            NEEDS REVIEW
          </span>
        );
      case "HIGH_RISK":
        return (
          <span className="animate-pulse rounded-full border border-rose-500/20 bg-rose-500/10 px-3 py-0.5 text-xs font-bold text-rose-400">
            HIGH RISK
          </span>
        );
      default:
    ...
    <div className="relative min-h-screen bg-slate-950 px-4 py-8 text-slate-100 sm:px-6 lg:px-8">
      ...
      <div className="mx-auto max-w-4xl space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Student: {assessment.student_name || "Unknown"} ({assessment.student_reg_number})
              </p>
            </div>
          </div>
        ...
        {assessment.flags && assessment.flags.length > 0 && (
          <div className="space-y-3 rounded-2xl border border-rose-500/20 bg-rose-950/20 p-5">
            <h3 className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-rose-400">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              {assessment.flags.map((flag) => (
                <div
                  key={flag.id}
                  className="glass-panel rounded-xl border border-slate-800 bg-slate-950/40 p-3.5"
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`py-0.2 rounded px-1.5 text-[9px] font-bold ${flag.severity === "HIGH" ? "bg-rose-500/10 text-rose-400" : flag.severity === "MEDIUM" ? "bg-amber-500/10 text-amber-400" : "bg-slate-900 text-slate-500"}`}
                    >
                      {flag.severity}
                    </span>
                  </div>
                  <p className="mt-1 text-xs text-slate-300">{flag.flag_reason}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          ...
          <div className="space-y-6 md:col-span-2">
            ...
            <div className="glass-panel space-y-6 rounded-2xl border border-slate-800 bg-slate-950/40 p-6">
              <h3 className="border-b border-slate-900 pb-3 text-xs font-bold uppercase tracking-wider text-slate-400">
                Biometrics & Coordinates
              </h3>
                  ...
                  <div className="grid grid-cols-2 gap-4 rounded-xl border border-slate-900 bg-slate-900/10 p-4 text-xs">
                    <div>
                      ...
                      <p className="font-semibold text-slate-200">
                        {locationVal.latitude?.toFixed(5)}, {locationVal.longitude?.toFixed(5)}
                      </p>
                    </div>
                    <div>
                      <p className="text-slate-500">Allowed Radius</p>
                      <p className="font-semibold text-slate-200">
                        {locationVal.is_within_radius ? "Within boundary" : "Outside boundary"}
                      </p>
                    </div>
                    <div>
                      <p className="text-slate-500">Distance from classroom center</p>
                      <p className="font-semibold text-slate-200">
                        {locationVal.distance_from_center
                          ? `${locationVal.distance_from_center.toFixed(1)} meters`
                          : "N/A"}
                      </p>
                    </div>
                    <div>
                      ...
                      <p className="font-semibold text-slate-200">
                        {locationVal.accuracy ? `${locationVal.accuracy.toFixed(1)}m` : "Unknown"}
                      </p>
                    </div>
                  </div>
                ) : (
                  <p className="rounded-xl border border-rose-500/10 bg-rose-500/5 p-3 text-xs text-rose-400">
                    Location verification metadata is missing.
                  </p>
                )}
              </div>
                ...
                {verificationSession ? (
                  <div className="grid grid-cols-2 gap-4 rounded-xl border border-slate-900 bg-slate-900/10 p-4 text-xs">
                    <div>
                      ...
                      <p
                        className={`font-semibold ${verificationSession.camera_granted ? "text-emerald-400" : "text-rose-400"}`}
                      >
                    ...
                    <div>
                      ...
                      <p
                        className={`font-semibold ${verificationSession.face_detected ? "text-emerald-400" : "text-rose-400"}`}
                      >
                    ...
                    <div>
                      ...
                      <p
                        className={`font-semibold ${verificationSession.blink_verified ? "text-emerald-400" : "text-rose-400"}`}
                      >
                    ...
                    <div>
                      ...
                      <p
                        className={`font-semibold ${verificationSession.smile_verified ? "text-emerald-400" : "text-rose-400"}`}
                      >
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-slate-200">
                        {verificationSession.attempt_count} attempts
                      </p>
                    </div>
                    <div>
                      <p className="text-slate-500">Biometric Status</p>
                      <p
                        className={`font-semibold uppercase ${verificationSession.liveness_passed ? "text-emerald-400" : "text-rose-400"}`}
                      >
                  ...
                  <p className="rounded-xl border border-rose-500/10 bg-rose-500/5 p-3 text-xs text-rose-400">
                    Liveness verification metadata is missing.
                  </p>
                )}
              </div>
            </div>
          ...
          <div className="space-y-6">
              ...
              {evidence ? (
                <div className="group relative overflow-hidden rounded-xl border border-slate-800 bg-slate-950">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="absolute bottom-2 right-2 flex items-center gap-1 rounded border border-slate-800 bg-slate-950/80 px-2 py-1 text-[10px] font-bold text-white hover:bg-slate-950"
                  >
                    <span>Full URL</span>
                ...
                <div className="flex aspect-[4/3] flex-col items-center justify-center rounded-xl border border-slate-800 p-4 text-center">
                  <Camera className="h-8 w-8 text-slate-700" />
                  <p className="mt-2 text-xs text-slate-500">Selfie was not uploaded</p>
                </div>
              )}
            </div>
            ...
            <div className="glass-panel space-y-3 rounded-2xl border border-slate-800 bg-slate-950/40 p-5">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold uppercase text-white">{record.status}</span>
              </div>
        ...
        <section className="glass-panel space-y-4 rounded-2xl border border-slate-800 bg-slate-950/40 p-6">
          <h3 className="flex items-center gap-2 border-b border-slate-900 pb-3 text-xs font-bold uppercase tracking-wider text-slate-400">
            <Layers className="h-4 w-4 text-indigo-400" />
            Admin Review Panel
          </h3>

          {assessment.reviewed ? (
            <div className="space-y-3 rounded-xl border border-slate-800 bg-slate-900/10 p-5 text-xs">
              ...
              <p className="font-medium text-slate-300">Notes added during review:</p>
              <div className="rounded border border-slate-900 bg-slate-950 p-3 font-mono text-[11px] leading-relaxed text-slate-400">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  className="min-h-[90px] w-full rounded-xl border border-slate-800 bg-slate-900 p-3 text-xs text-slate-100 focus:border-indigo-500 focus:outline-none"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-2.5 pt-2">
                <button
                  ...
                  className="cursor-pointer rounded-lg bg-rose-500 px-4 py-2.5 text-xs font-bold text-white hover:bg-rose-600 disabled:cursor-not-allowed disabled:opacity-40"
                >
                  Reject Attendance
                </button>
                <button
                  ...
                  disabled={submittingReview || !notes.trim()}
                  className="cursor-pointer rounded-lg bg-emerald-600 px-4 py-2.5 text-xs font-bold text-white hover:bg-emerald-500 disabled:cursor-not-allowed disabled:opacity-40"
                >
                  Approve Attendance
                </button>
              </div>
