...
function AuditDashboardContent() {
  ...
  const getRoleBadge = (role: string | null) => {
    if (!role) {
      return (
        <span className="inline-flex items-center gap-1 rounded border border-slate-800 bg-slate-900 px-1.5 py-0.5 text-[9px] font-bold text-slate-400">
    ...
    switch (role.toUpperCase()) {
      case "ADMIN":
        return (
          <span className="inline-flex items-center gap-1 rounded border border-indigo-500/20 bg-indigo-500/10 px-1.5 py-0.5 text-[9px] font-bold text-indigo-400">
      ...
      case "DEVELOPER":
        return (
          <span className="inline-flex items-center gap-1 rounded border border-amber-500/20 bg-amber-500/10 px-1.5 py-0.5 text-[9px] font-bold text-amber-400">
      ...
      case "STUDENT":
        return (
          <span className="inline-flex items-center gap-1 rounded border border-emerald-500/20 bg-emerald-500/10 px-1.5 py-0.5 text-[9px] font-bold text-emerald-400">
      ...
      default:
        return (
          <span className="inline-flex items-center gap-1 rounded border border-slate-500/20 bg-slate-500/10 px-1.5 py-0.5 text-[9px] font-bold text-slate-400">
            {role.toUpperCase()}
          </span>
        );
    }
  ...
  const getActionColor = (action: string) => {
    ...
    if (act.includes("OVERRIDDEN") || act.includes("UPDATED"))
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Track and monitor platform operations, security events, and database activity
              </p>
            </div>
          </div>
          <button
          ...
          <div className="space-y-6 md:col-span-3">
            ...
            <section className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              {loadingStats ? (
                Array.from({ length: 4 }).map((_, i) => (
                  <div
                    key={i}
                    className="glass-panel h-24 animate-pulse rounded-2xl border border-slate-800 bg-slate-950/40 p-5"
                  />
                ))
              ) : (
                <>
                  ...
                  <div className="glass-panel rounded-2xl border border-l-2 border-indigo-900/40 border-l-indigo-500 bg-slate-950/40 p-5">
                    <span className="text-[9px] font-bold uppercase tracking-wider text-slate-500">
                      Admin Actions
                    </span>
                  ...
                  <div className="glass-panel rounded-2xl border border-l-2 border-emerald-900/40 border-l-emerald-500 bg-slate-950/40 p-5">
                    <span className="text-[9px] font-bold uppercase tracking-wider text-slate-500">
                      System Events
                    </span>
            ...
            <section className="glass-panel space-y-4 rounded-2xl border border-slate-800 bg-slate-950/40 p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-0.5 text-[10px] text-slate-500">
                    Generate and download report files matching current filters
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                <button
                  ...
                  className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-slate-800 bg-slate-900/60 p-3 text-xs font-semibold text-slate-200 transition hover:border-slate-700 hover:bg-slate-900 disabled:opacity-50"
                >
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                ...
                <div className="relative flex items-center gap-2">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-slate-800 bg-slate-900 px-3 py-1.5 text-xs text-slate-100 focus:outline-none"
                  />
                </div>
            ...
            <section className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-950/60">
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center gap-3 py-20 text-center text-slate-100">
                  <AlertCircle className="h-10 w-10 animate-bounce text-rose-400" />
                  <p className="text-sm font-bold">Query Execution Failed</p>
                  <p className="max-w-sm text-xs text-slate-500">{error}</p>
                </div>
              ) : logs.length === 0 ? (
                <div className="flex flex-col items-center justify-center gap-2 py-20 text-center text-slate-500">
                  <Shield className="h-10 w-10 text-slate-800" />
                  <p className="text-sm font-bold text-slate-400">Audit Trail is Empty</p>
                  <p className="max-w-sm text-xs text-slate-600">
                    No activity records match your current filtering parameters.
                  </p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse text-left text-xs">
                    ...
                    <tbody className="divide-y divide-slate-900/60 text-slate-300">
                      {logs.map((log) => (
                        <tr key={log.id} className="transition hover:bg-slate-900/30">
                          <td className="text-slate-450 whitespace-nowrap px-5 py-4 font-mono text-[10px]">
                          ...
                          <td className="whitespace-nowrap px-5 py-4">
                            <span
                              className={`rounded-full border px-2 py-0.5 text-[9px] font-bold ${getActionColor(log.action_type)}`}
                            >
                              {log.action_type}
                            </span>
                          </td>
                          <td className="whitespace-nowrap px-5 py-4 text-slate-400">
                            {log.entity_type ? (
                              <span className="rounded bg-slate-900 px-1 py-0.5 font-mono text-[10px]">
                                {log.entity_type}{" "}
                          ...
                          <td
                            className="max-w-xs truncate px-5 py-4 text-slate-400"
                          ...
                          <td className="whitespace-nowrap px-5 py-4 text-right">
                            <Link
                              ...
                              className="inline-flex cursor-pointer items-center gap-1 rounded border border-slate-800 bg-slate-900 p-1.5 text-[10px] font-semibold text-slate-300 hover:bg-slate-800"
                            >
            ...
            {totalPages > 1 && !loading && (
              <div className="flex items-center justify-between border-t border-slate-900 pt-4">
                <div className="text-[10px] text-slate-500">
                  Showing logs {(page - 1) * size + 1} to {Math.min(page * size, totalLogs)} of{" "}
                  {totalLogs}
                </div>
                <div className="flex items-center gap-2">
                  <button
                    ...
                    disabled={page === 1}
                    className="hover:bg-slate-850 cursor-pointer rounded-lg border border-slate-800 bg-slate-900 p-1.5 text-slate-400 transition hover:text-white disabled:cursor-not-allowed disabled:opacity-40"
                  >
